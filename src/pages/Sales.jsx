        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
          <div className="relative flex-1 sm:flex-initial">
            ...
            <select
              ...
              className="w-full sm:w-auto rounded-xl border border-slate-300 bg-white py-2 pl-9 pr-8 text-xs sm:text-sm text-slate-900 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
            >
          ...
          <div className="relative flex-1 sm:w-64">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-300 bg-white py-2 pl-9 pr-3 text-xs sm:text-sm text-slate-900 placeholder:text-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
            />
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-xs overflow-hidden">
        ...
        {!loading && error && (
          <div className="p-8 sm:p-10 text-center">
            ...
            <button
              onClick={reload}
              className="inline-flex items-center gap-2 rounded-xl border border-slate-300 bg-white px-4 py-2 text-sm text-slate-600 shadow-xs transition hover:border-emerald-500 hover:text-slate-900"
            >
              <RefreshCw size={14} />
              ព្យាយាមម្តងទៀត
            </button>
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="p-12 sm:p-16 text-center">
            <Receipt size={40} className="mx-auto mb-4 text-slate-300" />
            <p className="text-sm text-slate-500">មិនទាន់មានការលក់នៅឡើយទេ</p>
          </div>
        )}
          ...
          <div className="divide-y divide-slate-100">
            {filtered.map((sale) => (
              <Link
                key={sale.id}
                to={`/sales/${sale.id}`}
                className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-4 sm:px-6 sm:py-4 transition hover:bg-slate-50 active:bg-slate-100"
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="truncate font-bold text-slate-900 text-sm">{sale.invoiceNumber}</p>
                    <span className="sm:hidden font-black text-emerald-600 text-sm ml-auto">
                ...
                <div className="flex items-center justify-between sm:justify-end gap-2.5 pt-2 sm:pt-0 border-t border-slate-50 sm:border-0">
                  <span className="hidden sm:block font-bold text-slate-900 text-sm mr-1">
                    {formatCurrency(sale.total)}
                  </span>
