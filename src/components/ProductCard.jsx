...
export default function ProductCard({ product, onAdd, cartQuantity = 0 }) {
  ...
  return (
    <div
      ...
      className={`group relative flex flex-col overflow-hidden rounded-2xl border bg-white dark:bg-slate-900 shadow-2xs transition-all duration-150 select-none ${
        outOfStock
    ...
    >
      ...
      <div className="relative flex aspect-[4/3] min-h-[100px] sm:min-h-[120px] w-full shrink-0 items-center justify-center bg-slate-50/80 dark:bg-slate-800/60 p-2 overflow-hidden">
      ...
      <div className="flex flex-1 flex-col p-2 sm:p-2.5">
        <p
          className="line-clamp-1 text-xs font-bold text-slate-800 dark:text-slate-100 transition-colors group-hover:text-emerald-700 dark:group-hover:text-emerald-400"
          title={product.name}
        >
          {product.name}
        </p>
        <p className="truncate text-[10px] text-slate-400 dark:text-slate-400">
          {product.sku ? `SKU: ${product.sku}` : '\u00A0'}
        </p>

        <div className="mt-auto flex items-center justify-between pt-1.5">
          <span className="text-xs font-black text-emerald-600 dark:text-emerald-400 sm:text-sm">
        ...
        <button
          ...
          disabled={outOfStock}
          className={`mt-2 flex h-7 items-center justify-center gap-1 rounded-xl text-xs font-bold text-white transition active:scale-95 disabled:cursor-not-allowed disabled:bg-slate-200 dark:disabled:bg-slate-800 disabled:text-slate-400 dark:disabled:text-slate-600 ${
            cartQuantity > 0
